  ...
  constructor(strip: number[], i: number) {
    ...
    switch (opCodeStr) {
      ...
      case "08":
        ...
        break;
      case "09":
        this.operation = Operation.AdjustRelativeBase;
        numberOfParameters = 1;
        break;
      case "99":
        ...
        break;
      default:
        throw new Error(`Unrecognized opcode: ${opCodeStr}`);
    }

    if (i + numberOfParameters >= strip.length) {
      throw new Error("Ran out of strip");
    }
    ...
    this.parameters = parameterValues.map((value, index) => {
      ...
      switch (parameterModeChar) {
        ...
        case "2": parameterMode = ParameterMode.Relative; break;
        default: throw new Error(`Unrecognized ParameterMode: ${parameterModeChar}`);
      }
